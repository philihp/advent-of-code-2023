...
console.log(
  JSON.stringify(
    actualGames
      .map((game) => {
        const [_, matches] = game.split(': ')
        return matches
          .split('; ')
          .map((match) =>
            match.split(', ').map((pair) => {
              const [n, foundColor] = pair.split(' ')
              return [foundColor, Number(n)]
            })
          )
          .reduce((accum, thoughts) => {
            return thoughts.reduce(
              (accum, [color, value]) => (accum[color] >= value ? accum : { ...accum, [color]: value }),
              accum
            )
          }, {})
      })
      .map((minSet) => Object.values(minSet))
      .map((n) => {
        console.log(n)
