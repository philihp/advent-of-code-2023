...
console.log(
  encoded
    .map((row) => [row, ...range({ to: row.length })])
    .map(([str, ...arr]) => arr.map((start) => str.substr(start)))
    .map((suffixes) =>
      suffixes.reduce(
        ([first, last], suffix) => {
          if (suffix.startsWith("zero")) return [first ?? 0, 0];
          if (suffix.startsWith("0")) return [first ?? 0, 0];
          if (suffix.startsWith("one")) return [first ?? 1, 1];
          if (suffix.startsWith("1")) return [first ?? 1, 1];
          if (suffix.startsWith("two")) return [first ?? 2, 2];
          if (suffix.startsWith("2")) return [first ?? 2, 2];
          if (suffix.startsWith("three")) return [first ?? 3, 3];
          if (suffix.startsWith("3")) return [first ?? 3, 3];
          if (suffix.startsWith("four")) return [first ?? 4, 4];
          if (suffix.startsWith("4")) return [first ?? 4, 4];
          if (suffix.startsWith("five")) return [first ?? 5, 5];
          if (suffix.startsWith("5")) return [first ?? 5, 5];
          if (suffix.startsWith("six")) return [first ?? 6, 6];
          if (suffix.startsWith("6")) return [first ?? 6, 6];
          if (suffix.startsWith("seven")) return [first ?? 7, 7];
          if (suffix.startsWith("7")) return [first ?? 7, 7];
          if (suffix.startsWith("eight")) return [first ?? 8, 8];
          if (suffix.startsWith("8")) return [first ?? 8, 8];
          if (suffix.startsWith("nine")) return [first ?? 9, 9];
          if (suffix.startsWith("9")) return [first ?? 9, 9];
          return [first, last];
        },
        [undefined, undefined]
      )
